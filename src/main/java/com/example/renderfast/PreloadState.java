...
public class PreloadState extends SavedData {
	...
	public static final SavedDataType<PreloadState> TYPE = new SavedDataType<>(
			Identifier.fromNamespaceAndPath(RenderFastMod.MOD_ID, "preload_state"),
			PreloadState::new,
			CODEC,
			null
	);
	...
	private PreloadState(boolean started, boolean completed, boolean paused, int centerX, int centerZ, int doneCount, String dimension) {
	...
	public static PreloadState get(MinecraftServer server) {
		...
		if (overworld == null) return new PreloadState();
